<div>
    <div class="d-flex justify-content-between">
        ...
        @if ($category->hasARange())
            <div class="d-flex">
                <input class="mr-4 form-control" wire:model="minimumOctaves" type="text" placeholder="Minimum Octaves">
                <input class="form-control" wire:model="maximumOctaves" type="text" placeholder="Maximum Octaves">
            </div>
        @endif
    </div>
    <table class="table mt-4 table-striped table-bordered">
        <thead>
            <tr>
                ...
                @if ($category->hasARange())
                    <th scope="col">Minimum Octaves</th>
                    <th scope="col">Maximum Octaves</th>
                @endif
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                @if ($category->hasARange())
                    <td>{{ $piece->minimum_octaves }}</td>
                    <td>{{ $piece->maximum_octaves }}</td>
                @endif
            </tr>
            @empty
            <tr>
                <td colspan="{{ $category->hasARange() ? '6' : '4' }}">
